// Code from: https://github.com/KinoLien/gitzip
...
(function() {
                        ...
                        [
                            ...
                            function() {
                                ...
                                onmessage = function(e) {
                                    ...
                                    if (
                                        pathTryQueue[pathTryQueue.length - 1] ==
                                        ''
                                    )
                                        pathTryQueue.pop();
                                    var results = {};
                                    while (!toBreak) {
                                        // case: release, release/2.1
    ...
    var _checkAndGetDownloadURL = function(url) {
        if (url) {
            if (githubDownloadUrl.test(url)) return Promise.resolve(url);
                ...
                return _githubUrlChecker.check(url).then(function(resolved) {
                    return (
    ...
    var _getContentOfGitUrl = function(url, params) {
        ...
        return _callAjax(url, params).then(function(xmlResponse) {
        ...
        return _callAjax(url, params).then(function(xmlResponse) {
            ...
            if (results.truncated) {
                progressCallback.call(
                    callbackScope,
                    'error',
                    'The tree travels is over than API limitation (500 files)'
                );
    ...
    function zipIt(zipName, url, callbackScope) {
        ...
        if (url && githubProvidedUrl.test(url)) {
            progressCallback.call(
                callbackScope,
                'prepare',
                'Fetching list of Dir contains files.'
            );
            var params = {};
            if (token) params['access_token'] = token;
            params['recursive'] = 1;
                ...
                .then(function(xmlResponse) {
                    ...
                    results.tree.forEach(function(item) {
                        if (item.type == 'blob') {
                            var p = {};
                            promises.push(
                                _getContentOfGitUrl(item.url, p).then(function(
                                    content
                                ) {
                                    var path = item.path;
                                    fileContents.push({
        ...
        _githubUrlChecker
            ...
            .then(function(resolved) {
                if (!resolved.path) {
                    ...
                    progressCallback.call(
                        callbackScope,
                        'prepare',
                        'Finding file/dir content path from resolved URL'
                    );
                    var params = {};
                        ...
                        _callAjax(
                            ...
                            .then(function(xmlResponse) {
                                ...
                                results.forEach(function(item) {
                                    if (item.type == 'dir') {
                                        ...
                                        promises.push(
                                            _getTreeOfGitUrl(item.git_url).then(
                                                function(results) {
                                                    // add currentPath
                            ...
                            .then(function(urls) {
                                var fetches = urls.map(function(item) {
                                    return _getContentOfGitUrl(item.url).then(
                                        function(content) {
                                            var path = item.path;
                                            progressCallback.call(
                                                callbackScope,
            ...
            .catch(function() {
                progressCallback.call(
                    callbackScope,
                    'error',
                    'Invalid URL: value is [' + pathToFolder.toString() + ']'
                );
                throw 'INVALID URL';
            });
    }
